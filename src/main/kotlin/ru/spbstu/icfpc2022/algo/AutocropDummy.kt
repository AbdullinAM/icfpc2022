...
class AutocropDummy(task: Task) : Solver(task) {
     ...
     */
    private fun scanright(color: Color, height: Int, width: Int, col: Int, f: PixelsExtractor, tolerance: Int): Int {
    ...
    fun autocrop(image: ImmutableImage, color: Color, colorTolerance: Int): Pair<Shape?, ImmutableImage> {
        val x1 =
            scanright(color, image.height, image.width, 0, { r -> pixels(image, r.x, r.y, r.width, r.height) }, colorTolerance)
        val x2 =
            scanleft(color, image.height, image.width - 1, { r -> pixels(image, r.x, r.y, r.width, r.height) }, colorTolerance)
        val y1 =
            scandown(color, image.height, image.width, 0, { r -> pixels(image, r.x, r.y, r.width, r.height) }, colorTolerance)
        val y2 =
            scanup(color, image.width, image.height - 1, { r -> pixels(image, r.x, r.y, r.width, r.height) }, colorTolerance)
        return when {
    ...
    private fun computeBlockAverage(image: ImmutableImage, start: Point, end: Point): Color {
        var r = 0L
        var g = 0L
        var b = 0L
        var a = 0L
    ...
    override fun solve(): List<Move> {
        ...
        while (true) {
            val width = 10
            val tolerance = 17
            val shape = state.state.canvas.blocks[state.block]!!.shape
            ...
            val colors = variants.map {
                computeMaxColor(state.image, it.first, it.second)
            }
            val autocrops = colors.map { it to autocrop(state.image, it, tolerance) }
            ...
            val colorMove = ColorMove(state.block, bestCrop.first)
            var newState = state.state.move(colorMove)
